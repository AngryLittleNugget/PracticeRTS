...
public class HexGridMeshGeneration : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        if (hexGrid == null)
        {
            Debug.LogError("He ain't got no hexGrid Lieutennant Dan");
        }
    }
        ...
        for (int z = 0; z < height; z++)
        {
            for (int x = 0; x < width; x++)
            {
                for (int s = 0; s < HexMetrics.Corners(hexSize, orientation).Length; s++)
                {
                    ...
                    triangles[3 * 6 * (z * width + x) + s * 3 + 0] = (z * width + 7) * 7;
                    triangles[3 * 6 * (z * width + x) + s * 3 + 1] = (z * width + 7) * 7 + s + 1;
                    triangles[3 * 6 * (z * width + x) + s * 3 + 2] = (z * width + 7) * 7 + cornerIndex;
                }
            }
        }
